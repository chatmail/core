...
ORDER BY id"
    ...
    /// Asserts many errors or warning should be logged, that can happen in any order.
    ///
    /// The first part of the passed tuples controls whether the log should be a warning (`false`),
    /// or an error (`true`).
    /// The second part of the tuple is the pattern used for matching.
    /// The last part of the tuple controls how many times the pattern must be hit.
    ///
    /// # Important
    ///
    /// Order of assertions matters: if a log can be matched by multiple patterns,
    /// the first one takes precedence.
    ///
    /// Delegates to [`InnerLogSink::assert_many`].
    pub async fn assert_many(&self, assertions: Vec<(bool, &str, usize)>) {
        self.log_sink.assert_many(assertions).await
    }
}
...
///
/// Tests redirect the stdout of the test thread and capture this,
/// printing the logs as they are received.
#[derive(Debug, Clone, Default)]
...
impl InnerLogSink {
    ...
    pub async fn assert_many(&self, mut assertions: Vec<(bool, &str, usize)>) {
            ...
            tokio::time::timeout(Duration::from_secs(1), self.events.recv()).await
        {
            ...
            'assertions: for (assert_is_error, pat, count) in &mut assertions {
                if *assert_is_error != is_error || *count == 0 {
                    continue 'assertions;
                }
                if let Some(log) = match is_error {
                    false => event.typ.get_warn(),
                    true => event.typ.get_error(),
                } && log.contains(*pat)
                {
        ...
        for (is_error, pat, count_left) in assertions {
            if count_left > 0 {
                if is_error {
