...
pub(crate) fn render_queued_mail(
    ...
    let date = chrono::DateTime::<chrono::Utc>::from_timestamp(timestamp, 0)
        .unwrap()
        .to_rfc2822();
    inner_headers.extend(b"Date: ");
    inner_headers.extend(date.as_bytes());
    ...
    if is_encrypted {
        ...
        outer_headers.extend(b"Date: ");
        outer_headers.extend(date.as_bytes());
        outer_headers.extend(b"\r\n");
    }
    ...
    if should_attach_pubkey {
        ...
        if is_encrypted {
            inner_headers.extend(b"Autocrypt: ");
            autocrypt_header.write_header(&mut inner_headers, 11)?;
        } else {
            outer_headers.extend(b"Autocrypt: ");
